...
for lab in "${labs[@]}"; do
  ...
  if ! make -C "${ROOT_DIR}/${lab}" clean all; then
    ...
    case "${lab}" in
      *lab09-*|*lab10-*)
        echo "If -m32 failed, install 32-bit build support:"
        echo "  sudo apt install gcc-multilib libc6-dev-i386"
        ;;
    esac
    failed=1
  fi
...
echo "Note: lab09 and lab10 require 32-bit build support."
